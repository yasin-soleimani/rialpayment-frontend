<template>
  <transition-group appear @before-enter="beforeEnter" @enter="enter">
    <button
      @click="selectedInternetTypeRightel(item)"
...
<script lang="ts">
import { SelectInternetTypeRightel } from "@/interface/internet/selectInternetTypeRightel";
import { defineComponent } from "vue";
import gsap from "gsap";
export default defineComponent({
  name: "InternetPackageRightel",
  ...
  methods: {
    selectedInternetTypeRightel(item: SelectInternetTypeRightel): void {
      this.setTypeOperatorActive(item);
    },
    setTypeOperatorActive(item: SelectInternetTypeRightel): void {
      this.setAllTypeOperatorActive();
      const index = this.internetPackageList.findIndex(
        (el: SelectInternetTypeRightel) => el.id === item.id
      );
      if (index > -1) {
