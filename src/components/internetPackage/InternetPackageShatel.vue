<template>
  <transition-group appear @before-enter="beforeEnter" @enter="enter">
    <button
      @click="selectedInternetTypeShatel(item)"
...
<script lang="ts">
import { SelectInternetTypeShatel } from "@/interface/internet/selectInternetTypeShatel";
import { defineComponent } from "vue";
import gsap from "gsap";
export default defineComponent({
  name: "InternetPackageShatel",
  ...
  methods: {
    selectedInternetTypeShatel(item: any): void {
      this.setTypeOperatorActive(item);
    },
    setTypeOperatorActive(item: SelectInternetTypeShatel): void {
      this.setAllTypeOperatorActive();
      const index = this.internetPackageList.findIndex(
        (el: SelectInternetTypeShatel) => el.id === item.id
      );
      if (index > -1) {
